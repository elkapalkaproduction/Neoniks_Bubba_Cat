...
-(void)ShareFacebook
{
    ...
    [mySLComposerSheet addImage:[UIImage imageNamed:@"icon-120.png"]];
...
-(IBAction)openMail:(id)sender
{
    if([MFMailComposeViewController canSendMail])
    {
        ...
        [[CCDirector sharedDirector] presentViewController:mailer animated:YES completion:nil];
    }
    else
    {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Note" message:@"Cannot send email." delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil];
		[alert show];
		[alert release];
    }
}
